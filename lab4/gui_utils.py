...
def run_commands(tree, measurements, filename):
    """Открывает файл с командами и применяет их."""
    file_path = filedialog.askopenfilename(
        title="Выберите файл с командами",
        filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")]
    )
    if file_path:
        updated = process_commands(file_path, measurements)
        measurements.clear()
        measurements.extend(updated)
        update_table(tree, measurements)
...
def create_interface(measurements, filename):
    ...
    tk.Button(frame, text="Выполнить команды",
              command=lambda: run_commands(tree, measurements, filename)).grid(
